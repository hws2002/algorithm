...
void calcMaxIndependentSet(int n, int m){
	...
	aVisited = vector<bool> (n, false);
    bVisited = vector<bool> (m, false);
    aChosen = vector<bool> (n, false);
    bChosen = vector<bool> (m, false);
	
	for(int i = 0; i < n; i++){
		if( aMatch[i] == -1 && aVisited[i] == false){
			dfsMaxIS(i);
		}
	}
	
	for(int i = 0; i < n; i++)
		if( aVisited[i] ) aChosen[i] = true;
	
	for(int i = 0; i < m; i++)
		if( !bVisited[i] ) bChosen[i] = true;
}    
