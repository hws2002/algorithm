...
int main(int argc, char* argv[]){
	...
	// take input and store into vector
	int N,M;
	...
	for (int i = 0; i< M; i++){
		...
		for(int i= 0; i < 101; i++){
			if (LADDER[i] != 0){
				cout<<"x, y = "<<i<<" "<<LADDER[i]<<endl;
			}
		}
		cout<<"SNAKES"<<endl;
		for(int i= 0; i < 101; i++){
			if (SNAKE[i] != 0){
				cout<<"x, y = "<<i<<" "<<SNAKE[i]<<endl;
			}
		}
	#endif
	// BFS
	...
	bool VISITED[101] = {false}; // 넣을때 한번만 1로 바꾸면 됨!
	VISITED[1] = true;
	...
	while(current_posi < 100){
		if(!Q[steps].empty()){
			...
			for(int i = 6; i >=1 ; i--){// if not, check every possible steps
				...
				// if the position is already in queue, or have been visitd before, we can skip it
			...
			steps++;
			#ifdef DEBUG_VISIT
				cout<<"Positions VISITED till "<<steps<<"-th step"<<endl;
				for(int i =0; i< 101; i++){
					if (VISITED[i] != 0)
						cout<<i<<" "<<endl;
				}
